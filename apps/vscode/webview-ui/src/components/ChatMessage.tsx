...
function ThinkingIndicator() {
  return (
    <div className="flex items-center gap-2 mt-1 text-blue-500/80 py-1">
      ...
      <span className="text-[11px] font-medium tracking-wide">思考中…</span>
    </div>
  );
}
...
function ForkButton({ turnIndex, className }: ForkButtonProps) {
  ...
  const doFork = async () => {
    ...
    try {
      ...
      toast.error(`分支会话失败：${error instanceof Error ? error.message : String(error)}`);
    } finally {
      setIsForking(false);
  ...
  return (
    <>
      ...
      <StreamingConfirmDialog
        ...
        description={
          isStreaming
            ? "当前会话仍在生成回复。分支会话将停止生成并从此节点创建新会话，是否继续？"
            : "将从此节点创建一个新的分支会话，该节点之后的所有消息会被移除。是否继续？"
        }
        confirmLabel="分支"
