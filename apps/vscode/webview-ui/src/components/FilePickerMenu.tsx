    ...
    <div className="rounded-md border bg-popover shadow-md overflow-hidden">
        ...
        <>
          {showMediaOption && onSelectMedia && (
            ...
            >
              ...
              <span className="text-xs">选择图片或视频…</span>
            </button>
          )}
          <button
          ...
          >
            ...
            <span className="text-xs">浏览文件夹…</span>
          </button>
        </>
      ) : (
        <>
          ...
          >
            ...
            <span className="text-xs">返回搜索</span>
          </button>
          {currentPath && (
            <button
      ...
      <div className="max-h-64 overflow-y-auto">
        {isLoading ? (
          ...
          <div className="px-2 py-4 text-center text-xs text-muted-foreground">{mode === "search" ? "未找到文件" : "空文件夹"}</div>
        ) : (
          items.map((item, idx) => {
