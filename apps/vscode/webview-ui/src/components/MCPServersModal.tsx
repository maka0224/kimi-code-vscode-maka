...
function validateForm(f: FormData): string | null {
  if (!f.name.trim()) return "名称为必填项";
  if (f.transport === "http" && !f.url.trim()) return "URL 为必填项";
  if (f.transport === "stdio" && !f.command.trim()) return "命令为必填项";
  return null;
}
        ...
        <>
          <div>
            ...
            <label className="flex items-center gap-1.5 mt-1.5 cursor-pointer">
              ...
              <span className="text-xs text-muted-foreground">需要 OAuth</span>
            </label>
          </div>
          <KeyValueFields
            label="请求头"
          ...
          />
          <div>
            <Label className="text-[10px] text-muted-foreground">Bearer Token 环境变量</Label>
            <Input
...
function ServerItem({ server, onDelete }: { server: MCPServerConfig; onDelete: () => void }) {
  ...
  const handleUpdate = async () => {
    try {
      ...
      setTestOutput(`更新失败：${error instanceof Error ? error.message : String(error)}`);
    }
  };
  ...
  const handleAction = async (action: () => Promise<any>) => {
    ...
    try {
      ...
      setExpanded(true);
      setTestOutput(`错误：${error instanceof Error ? error.message : String(error)}`);
    } finally {
      setIsLoading(false);
    }
  };
          ...
          {server.mutable === false ? (
            <p className="text-[10px] text-muted-foreground">
              {server.source === "plugin"
                ? `由插件 "${server.origin ?? ""}" 提供 — 请改为更新插件清单`
                : `定义于 ${server.origin ?? "项目配置文件"} — 请改为编辑该文件`}
            </p>
          ) : (
            <ServerForm data={form} onChange={setForm} onSubmit={() => { void handleUpdate(); }} onCancel={() => setExpanded(false)} submitLabel="更新" />
          )}
        </div>
      )}
    </div>
      ...
      <div className="fixed inset-0 z-40 flex flex-col bg-background">
        <div className="flex items-center justify-between px-3 py-2 border-b">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xs font-medium">MCP 服务器</h2>
          </div>
          ...
          <div className="max-w-2xl mx-auto px-3 py-3 space-y-4">
            ...
            {showAdd && (
              <div className="rounded-md border border-blue-500/5 p-2.5">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-xs font-medium">添加 MCP 服务器</span>
                </div>
                <ServerForm data={addForm} onChange={setAddForm} onSubmit={() => { void handleAdd(); }} onCancel={() => setShowAdd(false)} submitLabel="添加服务器" />
              </div>
            )}
            ...
            {mcpServers.length === 0 && !showAdd && (
              <div className="py-6 text-center">
                ...
                <p className="text-xs text-muted-foreground">未配置 MCP 服务器</p>
              </div>
            )}

            <div className="space-y-1.5">
              <h3 className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider">推荐</h3>
              ...
              {RECOMMENDED_MCP_SERVERS.every((s) => installedNames.has(s.id)) && (
                <p className="text-[10px] text-muted-foreground text-center py-2">已安装全部推荐服务器</p>
              )}
            </div>
          </div>
        </div>
      ...
      <AlertDialog open={deleteTarget !== null} onOpenChange={(open) => !open && setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>这将会从配置中移除 "{deleteTarget}"。此操作不可撤销。</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
