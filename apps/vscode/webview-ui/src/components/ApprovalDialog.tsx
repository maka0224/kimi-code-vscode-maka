...
export function ApprovalDialog() {
  ...
  const options = [
    { key: "approve", label: "批准", index: 1 },
    { key: "approve_for_session", label: "批准，仅限本会话", index: 2 },
    { key: "reject", label: "拒绝", index: 3 },
  ] as const;
    ...
    <div className={cn("mb-0.5 border border-blue-200 dark:border-blue-800 rounded-lg overflow-hidden bg-background flex flex-col shrink")}>
      <div className="p-2 space-y-2 flex-1 min-h-0 overflow-hidden flex flex-col">
        <div className="flex items-center justify-between shrink-0">
          <div className="text-xs font-semibold text-foreground">允许此 {req.action.toLowerCase()} 吗？</div>
          {hasDisplay && (
