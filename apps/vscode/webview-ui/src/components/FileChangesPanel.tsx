    ...
    <div className="group flex items-center gap-1.5 px-2.5 py-1.5 hover:bg-accent/50 text-xs">
      ...
      <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
        <Tooltip>
          ...
          <TooltipContent>查看变更</TooltipContent>
        </Tooltip>
        {!isStreaming && (
          <>
            <Tooltip>
              ...
              <TooltipContent>撤销变更</TooltipContent>
            </Tooltip>
            <Tooltip>
              <TooltipTrigger asChild>
              ...
              <TooltipContent>保留变更</TooltipContent>
            </Tooltip>
          </>
        )}
      </div>
...
export function FileChangesPanel({ changes }: FileChangesPanelProps) {
  ...
  const handleRevert = async (filePath?: string) => {
    ...
    try {
      ...
      toast.error(`无法撤销变更：${error instanceof Error ? error.message : String(error)}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleKeep = async (filePath?: string) => {
    ...
    try {
      ...
      toast.error(`无法保留变更：${error instanceof Error ? error.message : String(error)}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  if (!changes.length) {
    return <div className="px-2.5 py-3 text-xs text-muted-foreground text-center">暂无文件变更</div>;
  }
    ...
    <div className="flex flex-col bg-card shrink">
      ...
      <div className="flex items-center justify-between px-2.5 py-1.5 text-xs bg-muted/30">
        <div className="flex items-center gap-2">
          <span className="text-muted-foreground">{changes.length} 个文件</span>
