  ...
  return (
    <div className="relative group/codeblock">
      ...
      {shouldCollapse && (
        ...
        >
          {expanded ? "收起" : `展开 +${lines.length - maxLines}`}
        </button>
      )}
    </div>
  );
...
function IORow({ label, children }: { label: "IN" | "OUT"; children: React.ReactNode }) {
  const displayLabel = label === "IN" ? "输入" : "输出";
  return (
    <div className="flex flex-col @[420px]:flex-row gap-1 @[420px]:gap-0.5 py-2">
      <span className="shrink-0 w-8 text-xs text-muted-foreground font-medium">{displayLabel}</span>
...
function SetTodoListTool({ result }: ToolRendererProps) {
  ...
  if (!todoBlock || !todoBlock.items || todoBlock.items.length === 0) {
    return <div className="py-2 text-xs text-muted-foreground">{!result?.is_error && "待办列表已更新"}</div>;
  }
  return (
    <div className="py-1">
      ...
      {result && (
        <IORow label="OUT">
          {hasRichDisplay ? (
            ...
            <span className={cn("text-xs", !result.is_error ? "text-emerald-500" : "text-red-500")}>{!result.is_error ? "✓ 已写入" : formatOutput(result.output)}</span>
          )}
        </IORow>
      )}
    </div>
        ...
        <IORow label="OUT">
          {hasRichDisplay ? (
            ...
            <span className={cn("text-xs font-medium", !result.is_error ? "text-emerald-600 dark:text-emerald-500" : "text-destructive")}>
              {!result.is_error ? "✓ 替换成功" : formatOutput(result.output)}
            </span>
          )}
        </IORow>
      )}
    ...
    <div className="divide-y divide-border">
      <IORow label="IN">
        <span className="text-[11px] font-mono">
          {pattern}
          {directory && <span className="text-muted-foreground ml-1.5">在 {directory}</span>}
        </span>
      </IORow>
      {result && output && (
        <IORow label="OUT">
      ...
      {result && (
        <IORow label="OUT">
          {hasRichDisplay ? (
            ...
            <span className={cn("text-xs", !result.is_error ? "text-emerald-500" : "text-red-500")}>{!result.is_error ? "✓ 已完成" : "✗ 失败"}</span>
          )}
        </IORow>
      )}
    </div>
            ...
            <div className="mt-2 space-y-3">
              {subagentSteps.map((step) => (
                <div key={step.n} className="space-y-2">
                  <div className="text-[0.75rem] text-muted-foreground uppercase tracking-wider">步骤 {step.n}</div>
...
function getToolLabel(call: UIToolCall): string {
  ...
  switch (call.name) {
    case "Shell":
      return (args.command as string) || "命令";
    case "ReadFile":
      return (args.path as string)?.split("/").pop() || "文件";
    case "WriteFile":
      return (args.path as string)?.split("/").pop() || "文件";
    case "StrReplaceFile":
      return (args.path as string)?.split("/").pop() || "文件";
    case "Glob":
      return (args.pattern as string) || "模式";
    case "Task":
      return (args.description as string) || "子任务";
    case "SetTodoList":
      return "更新待办";
    default:
      return "";
