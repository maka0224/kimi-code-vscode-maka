...
export function InputArea({ onAuthAction }: InputAreaProps) {
  ...
  const completeSlashCommand = useMemoizedFn((name: string) => {
    if (!activeToken) return;
    const insert = `/${name} `;
  ...
  useEffect(() => {
    const onDragOver = (e: DragEvent) => {
      ...
      if (types.includes("Files") || types.includes("text/uri-list") || types.includes("application/vnd.code.tree.explorer")) {
  ...
  const isFileDrop = useMemoizedFn((e: React.DragEvent) => {
    const types = Array.from(e.dataTransfer?.types || []);
        ...
        try {
          ...
          if (Array.isArray(items)) {
            for (const item of items) {
              if (item && typeof item === "object" && typeof item.scheme === "string" && typeof item.path === "string") {
                const uri = item.scheme === "file" && !item.path.startsWith("/")
                  ? `file:///${item.path}`
      ...
      <div className="relative shrink-0">
        ...
        >
          ...
          <textarea
            ...
            onPaste={handlePaste}
            placeholder={isStreaming ? "继续追问…" : "向 Kimi Code Maka 提问…（/ 命令 · @ 文件 · Alt+K 代码）"}
            className={cn(
                    ...
                    <DropdownMenuTrigger asChild>
                      ...
                      >
                        ...
                        <span className="flex min-w-0 items-center text-xs">
                          <span className="truncate">{currentModelConfig?.name ?? "暂无可选模型"}</span>
