...
- **提示词优化**：输入框上方按钮栏一键优化提示词的排版、错句与病句（不改变原意），气泡内可选模型与思考模式（选择自动缓存）并支持自定义优化提示词；结果直接写回输入框，可一键还原原文。
- **界面调整**：全新 Logo、蓝紫主题、用量/额度并入输入框工具行等。

## 更新说明

- **当前版本：`0.3.0`**，基于官方扩展 `0.7.3`（[MoonshotAI/kimi-code](https://github.com/MoonshotAI/kimi-code)）。
- 独立版本号从 `0.1.0` 起；与上游的对应关系记录在本文档与扩展内 CHANGELOG 中。

### 0.3.0

- 特性：提示词优化——输入框上方新增按钮栏（提示词优化居右），气泡内选择模型与思考模式（选择自动缓存，不跟随全局切换）并可编辑优化提示词；点击优化直接将结果写回输入框，工具行出现回退按钮可一键还原原文；请求为宿主侧一次性调用（kosong 直连所选模型），不进会话、不写入历史。
