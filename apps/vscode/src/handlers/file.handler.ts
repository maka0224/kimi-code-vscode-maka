...
interface ResolveDroppedUrisParams { uris: string[] }
interface ResolveDroppedUrisResult { mentions: string[]; skipped: number }
...
const openFileDiff: Handler<FilePathParams, { ok: boolean }> = async ({ filePath }, ctx) => {
  ...
  await vscode.commands.executeCommand(
    ...
    baselineUri,
    resolved.uri,
    `${path.basename(resolved.relativePath)} (changes from Kimi Code Maka)`,
  );
  return { ok: true };
};
...
const resolveDroppedUris: Handler<ResolveDroppedUrisParams, ResolveDroppedUrisResult> = async ({ uris }, ctx) => {
  if (!ctx.workDirUri) return { mentions: [], skipped: 0 };
  ...
  for (const uriString of uris) {
    ...
    const relativePath = relativeWorkspacePath(ctx.workDirUri, uri);
    ...
    try {
      ...
      mentions.push(isDirectory ? `${relativePath}/` : relativePath);
    } catch {
      skipped++;
    }
  }
...
export const fileHandlers: Record<string, Handler<any, any>> = {
  ...
  [Methods.ResolveDroppedUris]: resolveDroppedUris,
};
