  ...
  constructor(
    ...
    try {
      ...
      const rollbackHint = useAgentCoreV1
        ? ""
        : " You can roll back to the legacy engine: enable the 'maka.useAgentCoreV1' setting and reload the window.";
      throw new Error(
        `Failed to start the Kimi Code Maka engine: ${error instanceof Error ? error.message : String(error)}.${rollbackHint}`,
        { cause: error },
      );
    }
