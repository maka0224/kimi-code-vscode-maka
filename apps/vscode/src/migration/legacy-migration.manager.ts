      ...
      prompt:
        sources.length === 0
          ...
          : {
              ...
              message:
                "Legacy Kimi data was found. Migrate config, MCP servers, history, skills, and sessions into Kimi Code Maka? Your old data will be kept.",
              actions: [
    ...
    if (shareDir.kind === "invalid") {
      ...
      if (sourceHome !== undefined && samePath(sourceHome, this.targetHome)) {
        warnings.push({
          ...
          sourceHome,
          message: "The legacy KIMI_SHARE_DIR resolves to the Kimi Code Maka home and was ignored.",
        });
      } else if (
  ...
  for (const source of sources) {
    for (const failure of source.failures) {
      if (failure.code === "run-failed") {
        actions.push(
          `Fix access to ${source.source.sourceHome} or the Kimi Code Maka home, then run “Kimi Code Maka: Migrate Legacy Data” again.`,
        );
      } else {
        actions.push(
