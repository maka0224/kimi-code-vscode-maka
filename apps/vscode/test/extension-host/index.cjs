...
const EXTENSION_ID = "maka.kimi-code-maka";
const EXPECTED_COMMANDS = [
  "maka.clearAllState",
  "maka.focusInput",
  "maka.insertMention",
  "maka.logout",
  "maka.migrateLegacyData",
  "maka.newConversation",
  "maka.openInSideBar",
  "maka.openInTab",
  "maka.resetKimi",
  "maka.showLogs",
...
exports.run = async function run() {
  ...
  const config = vscode.workspace.getConfiguration("maka");
  ...
  await vscode.commands.executeCommand("maka.openInTab");
  await waitFor(() => {
    return vscode.window.tabGroups.all.some((group) =>
      group.tabs.some((tab) =>
        tab.input instanceof vscode.TabInputWebview && isKimiPanelViewType(tab.input.viewType)));
  }, 5_000, () => `Kimi Code Maka webview tab did not open; tabs=${describeTabs()}`);

  await vscode.commands.executeCommand("maka.showLogs");
  await vscode.commands.executeCommand("maka.resetKimi");
...
function isKimiPanelViewType(viewType) {
  ...
  return viewType === "makaPanel" || viewType.endsWith("-makaPanel");
}
