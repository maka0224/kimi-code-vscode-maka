...
const canCreateSymlink = await (async () => {
  const target = join(tmpdir(), "symlink-cap-target-" + Date.now());
  const link = join(tmpdir(), "symlink-cap-link-" + Date.now());
  try {
...
const itWithSymlink = it.skipIf(!canCreateSymlink);
...
describe("Webview workspace paths (selected-directory containment)", () => {
  ...
  itWithSymlink("omits a symlink when its target is outside the selected working directory", async () => {
    const workDir = join(root, "project");
  ...
  itWithSymlink("refuses to open a symlink whose target lies outside the selected working directory", async () => {
    const workDir = join(root, "project");
  ...
  itWithSymlink("omits an outside symlink when an SDK Write event requests baseline capture", async () => {
    const workDir = join(root, "project");
