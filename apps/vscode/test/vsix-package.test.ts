...
describe('Extension Development Host setup (isolated local state)', () => {
  it('creates the complete isolated directory layout for a debug launch', async () => {
    ...
    await expect(readFile(join(baseDir, 'workspace', 'README.md'), 'utf8')).resolves.toContain(
      'Isolated Kimi Code Maka extension development workspace',
    );
  ...
  it('keeps kimi-home credentials across re-prepares while resetting the rest', async () => {
    const parent = await makeTempDir('kimi-dev-profile-');
    const baseDir = join(parent, 'vscode-extension-dev');

    expect(runNode(prepareDevScript, ['--base-dir', baseDir]).status).toBe(0);
    const credentialFile = join(baseDir, 'kimi-home', 'credentials', 'kimi-code.json');
    await mkdir(dirname(credentialFile), { recursive: true });
    await writeFile(credentialFile, '{}');
    await writeFile(join(baseDir, 'workspace', 'stale.txt'), 'stale');

    expect(runNode(prepareDevScript, ['--base-dir', baseDir]).status).toBe(0);

    await expect(readFile(credentialFile, 'utf8')).resolves.toBe('{}');
    await expect(readFile(join(baseDir, 'workspace', 'stale.txt'), 'utf8')).rejects.toThrow();
  });
});
