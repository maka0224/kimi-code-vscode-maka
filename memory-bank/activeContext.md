...
**0.3.3 批次已提交并打包**：编辑器上下文 chip 右键菜单 + 欢迎页固定指南 + chip 空选区行号修复。文档（README×2、CHANGELOG）、图谱（fast 重建 39144 节点）、memory-bank 均已同步；扩展测试 401 全过、typecheck 通过。
...
1. 手动验证 0.3.3 交互（chip 右键菜单/追加至光标处、空选区 chip 只显示文件名、欢迎页固定指南）
2. 发布 0.3.1 ~ 0.3.3 vsix（`publish:vsix` / `publish:ovsx`）
